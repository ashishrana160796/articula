...
def calculate_intrinsic_dim_estimates(data_path: str | Path, save_path: str | Path, file_name: str | Path, subset_fraction: Optional[float]=1.):
    ...
    intrinsic_dim_df = pd.read_csv(save_path / file_name)
    ...
    print(intrinsic_dim_df.columns, intrinsic_dim_df.shape)
    intrinsic_dim_df.to_csv(save_path / file_name, index=False)

if __name__ == '__main__':
    calculate_intrinsic_dim_estimates(const.INTRINSIC_DIM_DATASET, const.TRANSFORMED_DATA_SAVE_PATH, const.TRANSFORMED_DATA_CSV_NAME)
