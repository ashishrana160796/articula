...
def create_base_corpus(savepath: str | Path, file_name: str):
    ...
    return brown_data_df.to_csv(Path(savepath) / file_name, index=False)


def generate_gpt_text(data_path: str | Path, file_name: str):
    ...
    return data_df.to_csv(Path(data_path) / file_name, index=False)

if __name__ == '__main__':
    create_base_corpus(const.TRANSFORMED_DATA_SAVE_PATH, const.GEN_EN_DATA_CSV_NAME)
    generate_gpt_text(const.TRANSFORMED_DATA_SAVE_PATH, const.GEN_EN_DATA_CSV_NAME)
